...
describe("Test ERC721Parser", () => {
    ...
    describe("Test getERC721Contract", () => {
        it("Should successfully parse ERC721 compatible contract", async () => {
                ...
                const ERC721Address = "0xeda8b016efa8b1161208cf041cd86972eee0f31e"
                const ERC721contract = await getERC721Contract(ERC721Address)

                expect(ERC721contract).to.have.property("name").eql("I HOUSE TOKEN")
                expect(ERC721contract).to.have.property("symbol").eql("IHT")
                expect(ERC721contract).to.have.property("decimals").eql("18")
                expect(ERC721contract).to.have.property("totalSupply").eql("1000000000000000000000000000")

        })
    })

    describe("Test getContractName", function() {
        const getContractName = new ERC721Parser().getContractName
        const tests = [
    ...
    describe("Test getContractSymbol", () => {
        const getContractSybol = new ERC721Parser().getContractSymbol
        const tests = [
    ...
    describe("Test getContractDecimals", () => {
        const getContractDecimals = new ERC721Parser().getContractDecimals
        const tests = [
    ...
    describe("Test getContractTotalSupply", () => {
        const getContractTotalSupply = new ERC721Parser().getContractTotalSupply
        const tests = [
